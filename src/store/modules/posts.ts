...
export default {
  ...
  actions: {
    createPost({ commit, state, rootState }: any, post: any) {
      const postId: any = firebase
        .database()
        .ref("posts")
        .push().key;
    ...
    updatePost({ state, commit, rootState }: any, { id, text }: any) {
  ...
  mutations: {
    setPost(state: any, { post, postId }: any) {
